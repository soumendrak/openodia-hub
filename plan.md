# OpenOdia search improvement plan

## Outcome

Replace the current partial command-palette lookup with a complete, predictable search system across OpenOdia pages, repositories, tools, models, datasets, papers, tutorials, and events. A known record must remain findable regardless of its source position; Odia- and English-language queries must follow documented normalization and ranking rules; partial source failures must be visible without taking healthy results down; and search results should prefer stable OpenOdia destinations over immediately sending visitors off-site.

This is an implementation plan only. It does not authorize deployment or changes to upstream catalog data.

## Status

Completed on 2026-09-21. Tasks 006–010 and their acceptance criteria are implemented and verified. Automated budgets plus live browser, accessibility, responsive, and endpoint evidence are recorded in [`docs/search-quality.md`](docs/search-quality.md). Deployment remains a separate explicit decision.

## Current-state diagnosis

- The global palette downloads repositories, Awesome entries, videos, and events on first open, then lets `cmdk` filter them in the browser.
- Each source is sliced to its first 30 records before matching, so later records are impossible to find.
- Models, the Hugging Face dataset registry, and papers are not part of the global result corpus.
- Ranking is `cmdk`'s generic fuzzy score over manually concatenated strings; there is no OpenOdia-specific field weighting, Odia normalization, transliteration, or synonym policy.
- Loading and source failures are rendered as missing groups, with no explanation to the visitor.
- Most content selections open upstream sites even when OpenOdia already has a stable resource permalink.
- Route searches use separate field lists and matching rules; Events only searches pages already loaded in the browser.
- Existing command-palette tests mock out the matching library, so they do not verify real query behaviour, ranking, completeness, or pre-filter caps.

## Scope

### In scope

- One typed search document/result contract shared by indexing, API responses, and UI consumers.
- Complete coverage of static pages, the unified resource catalog, papers, tutorials, and curated/live events.
- Deterministic normalization and relevance ranking for English, Odia script, common `Odia`/`Oriya` aliases, and an explicitly tested Romanized-Odia path.
- A cached server-side search service that tolerates individual source failures and never fetches every upstream again for every keystroke.
- Search result limits applied after scoring and grouping.
- Visible loading, partial-results, error, empty, and retry states.
- Internal OpenOdia destinations as the primary action wherever a stable page or permalink exists.
- Shared behaviour for route-level search, including URL-backed queries where appropriate.
- Unit, integration, real-component, browser, accessibility, and performance coverage.

### Out of scope

- A hosted third-party search product or vector database.
- Semantic/LLM-generated answers, embeddings, personalization, or query-history storage.
- Creating full internal detail pages for videos, events, or papers solely for this project; scoped route links and stable anchors are sufficient.
- Replacing upstream ingestion, correcting upstream metadata, or removing existing facets.
- Deployment.

## Delivery tasks

| Order | Task                                                                                     | Depends on               | Demonstrable result                                                          |
| ----- | ---------------------------------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------- |
| 1     | [006 Search contract, normalization, and ranking](issues/006-search-contract-ranking.md) | —                        | Pure, deterministic search core with Odia-aware fixtures                     |
| 2     | [007 Unified cached search service](issues/007-unified-search-service.md)                | 006                      | One endpoint searches every supported source and reports partial failures    |
| 3     | [008 Global command-palette experience](issues/008-global-search-palette.md)             | 007                      | Complete ranked results with honest async states and internal navigation     |
| 4     | [009 Route-search consistency and deep links](issues/009-route-search-consistency.md)    | 006; coordinate with 007 | Directory and content routes use the same query semantics and linkable state |
| 5     | [010 Search quality gate](issues/010-search-quality-gate.md)                             | 007–009                  | Regression, accessibility, completeness, and performance gates pass          |

Tasks 3 and 4 can proceed in parallel after the service contract is stable. Task 5 blocks completion.

## Target architecture

1. Source adapters emit a common `SearchDocument` containing stable identity, kind, title, summary, weighted keywords, internal destination when available, upstream destination, and source metadata.
2. A pure search core normalizes the query and documents, calculates deterministic field-weighted scores, deduplicates stable identities, and returns ranked matches.
3. A server-side search service builds or refreshes a cached snapshot from existing cached loaders. Individual adapters settle independently so one unavailable upstream produces marked partial results rather than an empty search.
4. `/api/search` validates a bounded query and filters the complete snapshot. Global and per-group limits are applied after scoring.
5. The command palette debounces and cancels requests, renders the server's order without applying a second hidden fuzzy filter, and exposes loading/partial/error states.
6. Route-level searches reuse the same normalization and field policy. Their `q` state becomes linkable, and scoped server search is used wherever client pagination would otherwise hide valid results.

## Required relevance contract

The implementation must document and test the ordering. The initial contract is:

1. Exact normalized title/name match.
2. Title/name prefix and complete-token matches.
3. Stable identifier, author/community/channel, task/category, tag, and known alias matches.
4. Description/abstract matches.
5. Bounded typo-tolerant fallback, only when it produces a meaningful score.

Exact and title matches must not be displaced by incidental description text. Stable tie-breakers must prevent result order from moving between identical requests. Generic fuzzy subsequences must not create obviously unrelated matches.

## Data and resilience contract

- Use the existing unified catalog for repositories, tools, models, and datasets so overlapping upstream entries resolve to one result.
- Index all loaded records; never truncate a source before matching.
- Deduplicate by internal permalink or canonical upstream identity before ranking.
- Reuse source caches and maintain a search snapshot with an explicit freshness policy. A typed query must not trigger a fresh GitHub/Hugging Face/YouTube/OpenAlex/Bevy fan-out on every keystroke.
- Return source status and snapshot time alongside results. Healthy sources remain usable when another source is stale or unavailable.
- Bound query length, response size, and result count. Do not log or send raw search terms to analytics.

## UX contract

- Preserve the lazy-loaded palette and `⌘K`/`Ctrl+K` entry points.
- Show an immediate local Pages group while remote results load.
- Debounce remote queries and cancel superseded requests.
- Distinguish `Loading`, `Partial results`, `Could not search`, and `No matches`.
- Keep keyboard selection, focus restoration, screen-reader labels, and mobile scrolling correct.
- Navigate to an OpenOdia permalink or scoped route first. Offer the upstream URL as a secondary action where useful.
- Explain searchable scope in the input/help text; do not imply that unavailable sources were searched successfully.

## Verification gates

### Functional fixtures

- A matching record beyond position 30 is returned.
- Models, datasets, and papers appear in global results.
- Exact-title matches rank above description-only and fuzzy matches.
- `Odia` and `Oriya` aliases behave consistently.
- Agreed Odia-script and Romanized-Odia fixtures find the same intended records.
- Duplicate catalog entries collapse to one stable result.
- One failed source yields labelled partial results while healthy-source matches remain selectable.
- Event search finds a valid event that has not been loaded into the visible paginated timeline.
- Internal permalinks are selected where they exist.
...
- `pnpm build`
- `just coverage`
- Browser verification at 320px, 390px, tablet, and desktop widths.
- Keyboard-only and screen-reader-state review of the palette.
- Warm-search latency and response-size budgets recorded in the quality task before implementation is declared complete.

## Rollout gates and stop conditions

1. Land the pure contract and fixtures before introducing the endpoint. Stop if the team cannot agree on representative Odia/English queries and expected ordering.
2. Prove the cached service does not produce an upstream request storm. Stop integration if one typed query can trigger uncached fan-out across every provider.
3. Put the palette behind the new endpoint only after completeness, partial-failure, and ranking tests pass.
4. Do not remove the existing route-search paths until parity fixtures pass for each route.
5. Deployment remains a separate explicit decision after production-like browser and latency checks.

## Definition of done

A visitor can search the complete supported OpenOdia corpus using predictable English or Odia-aware queries, including records that previously fell outside the first 30. Results are deduplicated, ranked by a documented contract, honest about source availability, keyboard and screen-reader usable, and routed to stable OpenOdia pages where possible. Route-level searches follow the same semantics, and automated tests exercise the real filtering and async behaviour rather than only mocked presentation components.

---

## Legacy Pattachitra implementation plan

The earlier visual implementation plan follows for historical and maintenance context. It is not part of the search improvement scope above.
...
| #   | Change                                                                                                                                                                                                                                                                                                         | Where                                                 |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| R1  | **Replace the pillars.** "YouTube channel / PyPI package / Tools directory" → ecosystem pillars, e.g. **Tools & libraries · Models & datasets · Community & learning**. The package and channel become _entries inside_ those pillars, styled identically to OdiaGenAI, OdiaWikimedia, imsbg's apps, etc.      | `src/routes/index.tsx:175`, `src/routes/about.tsx:47` |
...
| C2  | **Event merge clobbers curated data.** `{...existing, ...e}` merges D1 events over curated ones by URL, and D1 rows always carry `location: undefined` / `description: ""` as _present_ keys — so hand-enriched venue lines and descriptions are overwritten with nothing.                                                                                                                                                                                                       | `src/routes/events.tsx:224`, `events-feed.ts:133`, `src/lib/events-store.ts:103` |
...
| #   | Defect                                                                                                                                                                                                                            | Location                                            |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| C13 | Awesome description cleaner deletes markdown link _text_ instead of keeping it (`"$1"` missing) — the visible "code()" artifacts and missing words on /tools cards.                                                               | `src/routes/api/awesome.ts:78`                      |
...
7. **Odia NLP benchmark/leaderboard page** — task × dataset × metric table seeded from
   ...
   there is _no_ current home for Odia leaderboards. This is the highest-differentiation
   feature on the list.
...
10. **Contrast audit** in both themes, checking English and Odia text separately (Odia glyphs
    ...
    homepages.
11. **Command-palette discoverability**: put a visible "⌘K" hint inside the search _input_ on
...
| B2  | Locale toggle visible at every width, labeled in the _target_ language (`ଓଡ଼ିଆ` / `English`, USWDS pattern), with `lang` on the label.                                                                                                                                                                                                 |
...
| #   | Found by                                                                               | Fix                                                                                                                                                                                                                                |
| --- | -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B13 | V6 — the new hero stat read exactly "200", because the HF fetch was capped at one page | Followed HF's `Link: rel="next"` pagination. Models 200 → 800, datasets 200 → 457. Where the page cap still bites, the count renders as "800+" and the registry says so in words — B13's complaint was the _silent_ cap.           |
...
- The mobile filter _drawer_ from UX 3 (Baymard's "Show N results" pattern) was not built —
...
| #          | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| UX 5       | **Per-resource permalinks.** `/r/{gh\|model\|dataset}/{owner}/{name}` — SSR'd, canonical-tagged, with `Dataset` / `SoftwareSourceCode` JSON-LD, license, freshness dates, tags, BibTeX/APA, and a "Copy link" button. Identity is derived from the URL (`lib/resource-id.ts`), so a curated Awesome row and the repo it points at resolve to the _same_ permalink instead of being two unrelated cards. Directory cards now link to the permalink; entries that resolve to nothing (arXiv, project homepages) still link straight out. |
...
| —                | V6 — the "Corpus stats" playground sample threw `TypeError: 'int' object is not callable` | `FreqDist.total_count` and `.ttr` are properties, not methods. Caught only by _running_ the sample in the browser; reading the signatures was not enough. All three new samples now execute and produce correct Odia output.                                       |
...
| #          | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Feature 7  | **`/leaderboard` — Odia benchmark results.** One table per `<task, dataset, metric>` (the Papers-with-Code shape), sorted the right way per metric (WER/CER ascending, accuracy descending), one best score per model, each linking to its permalink. **40 results across 10 benchmarks**, read from the `model-index` block of the 300 most-downloaded Odia-tagged model cards. A "How to read this" panel sits _above_ the numbers and states that every score is self-reported, that HF-verified scores carry a badge, that scores compare only within one table, and that paper results are deliberately absent. |
...
| —   | V5 at 1024px — the header rendered 1156px of tabs inside a 975px box | Ten nav tabs never fit: the header is capped at `max-w-6xl` (≈1118px inner), so the row would overflow at _every_ width and was simply being clipped. Desktop row now starts at `xl` and "Add project" is `menuOnly` (still in the menu, footer, and ⌘K). Nine tabs, fits with headroom.                                                |
...
**The bug.** Facet counts were computed over the _unfiltered_ list, so they went
...
**The fix.** `lib/facets.ts` now computes each facet against the _other_ facets'
...
Zero options sort last, a _selected_ option is never disabled (you must be able
to undo what emptied the page), and counts are set in tabular figures so a chip
does not change width when 44 becomes 8.

`/treebank` had the same bug plus a subtler one: its facets counted _tokens_
while its results are _sentences_. Both counts are now cross-filtered and
...
| ----- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | Add structured GitHub forms for an event/organizer submission and a correction. | Both forms set an explicit title prefix and existing suitable label; canonical and registration URLs are separate; required fields accept an explicit unknown where facts are absent. |
